...
class Go1AdaptationDr(LeggedRobot):
    ...
    def compute_observations(self):
        ...
        self.obs_history = torch.cat(
            (single_obs[:, 3:48][:, None, :], self.obs_history[:, :-1, :]), dim=1
        )
        # self.obs_buf = torch.cat((self.obs_history.flatten(1, 2), single_obs[:, 3:48]), dim=1)
