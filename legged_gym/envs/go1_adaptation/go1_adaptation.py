...
class Go1Adaptation(LeggedRobot):
    ...
    def get_observations(self):
        ...
        obs_dict = {
            ...
            # 'properties': self.disturbance_force[:, 0, :] + (torch.zeros_like(self.disturbance_force[:, 0, :]).uniform_(-1., 1.) * self.cfg.noise.noise_scales.disturbance if self.add_noise else 0),
            'current_obs': self.obs_buf,
            'supervised_latent_ground_truth': self.supervised_latent_ground_truth,
        }
        return obs_dict, self.extras
    ...
    def compute_observations(self):
        ...
        self.obs_buf = single_obs[:, 3:48].clone()
        self.obs_history = torch.cat(
            (self.obs_buf[:, None, :], self.obs_history[:, :-1, :]), dim=1
        )
        ...
        self.privileged_obs_buf = single_obs[:, :-4].clone()
        self.supervised_latent_ground_truth = torch.cat((single_obs[:, 0:3], single_obs[:, -7:-4]), dim=1)
