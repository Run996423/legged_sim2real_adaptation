...
from rsl_rl.modules import ActorCriticVAE
from rsl_rl.storage import RolloutStorageForPPOVAE
...
class PPOVAE:
    ...
    def __init__(
        ...
        self.storage: RolloutStorageForPPOVAE = None  # type: ignore
        self.transition = RolloutStorageForPPOVAE.Transition()
    ...
    def init_storage(
            ...
            current_obs_shape, privileged_obs_shape, actions_shape
    ):
        # create rollout storage
        self.storage = RolloutStorageForPPOVAE(
            num_envs,
    ...
    def act(self, obs_dict, critic_obs):
        ...
        obs_history = obs_dict["obs_history"]  # (batch_size, history_length + 1, obs_dim)
        current_obs = obs_dict["current_obs"]  # (batch_size, obs_dim)
        # compute the actions and values
        action_sampled, _, _, _ = self.policy.act(obs_dict, compute_vae_loss=False)
    ...
    def update(self):  # noqa: C901
        ...
        for (obs_history_batch, current_observations_batch,
            ...
            # -- actor
            action_sampled, vae_loss, recon_loss, kl_loss = self.policy.act(obs_dict={
            ...
            loss = self.vae_loss_coef * vae_loss + surrogate_loss + self.value_loss_coef * value_loss - self.entropy_coef * entropy_batch.mean()
